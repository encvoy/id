...
export const ViewWidget: FC = () => {
  ...
  const smallProviders = providers
    .filter(
      (provider) =>
        provider.type !== EProviderType.CREDENTIALS &&
        provider.groupe === "SMALL"
    )
  ...
  const localizedWidgetTitle = getLocalizedTextValue(
    client?.widget_title,
    i18n.language
  );
  const resolvedWidgetTitle = (localizedWidgetTitle || localizedClientName)
    .replace("WIDGET_APP_NAME", localizedClientName)
    .replace("APP_NAME", localizedClientName);
    ...
    >
      <Box className={styles.widget}>
        ...
        <div className={styles.header}>
          ...
          <p className={styles.title}>{resolvedWidgetTitle}</p>
          <IconButton className={styles.iconButton}>
        ...
        <div className={styles.main}>
          ...
          {!!providers.find(
            (provider) => provider.type === EProviderType.CREDENTIALS
          ) && (
            <>
        ...
        {!client?.hide_widget_footer && !!copyright && (
