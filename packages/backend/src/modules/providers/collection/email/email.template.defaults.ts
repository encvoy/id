...
export type TEmailTemplateDefault = {
  locale: ELocales;
...
export const getDefaultEmailTemplate = (
  locale: ELocales,
  action: NotificationAction,
) => DEFAULT_EMAIL_TEMPLATES.find((template) => template.locale === locale && template.action === action);

export const DEFAULT_EMAIL_TEMPLATES: TEmailTemplateDefault[] = [
  {
    locale: ELocales.ru,
