---
title: "Integración de Sentry con {{projectName}} — Configuración de SSO"
description: "Aprenda cómo configurar el inicio de sesión único para Sentry a través de {{projectName}}: configuración sencilla, protección de datos y acceso fluido para todos los empleados de la empresa."
keywords: 
keywords:
  - integración de Sentry con {{projectName}}
  - Sentry {{projectName}}
  - Sentry SSO
  ...
  - OAuth Sentry
  - iniciar sesión en Sentry a través de {{projectName}}
  - configuración de Sentry con {{projectName}}
  - conectar Sentry a {{projectName}}
...
# Cómo configurar la integración de Sentry con {{projectName}}

En esta guía, aprenderá cómo configurar el Inicio de Sesión Único (SSO) para **Sentry** utilizando el sistema **{{projectName}}**.
...
1. Inicie sesión o regístrese en **{{projectName}}**.  
...
2. Después de instalar el proveedor, edite el archivo de configuración de **Sentry** `sentry.conf.py`. En el archivo de configuración, añada un bloque de variables con los parámetros **OIDC_CLIENT_ID** y **OIDC_CLIENT_SECRET** copiados de la aplicación **{{projectName}}**.
    ...
    OIDC_CLIENT_ID = "client id de la aplicación {{projectName}}"
    OIDC_CLIENT_SECRET = "client secret de la aplicación {{projectName}}"
    OIDC_SCOPE = "openid email profile"
    OIDC_DOMAIN = "https://<dirección de {{projectName}}>/api/oidc"
...
3. Vaya al panel de administración de **Sentry** en `https://<ruta a Sentry>/settings/sentry/` y seleccione la sección **Auth**. Luego seleccione la aplicación **{{projectName}}**.

    <img src="./images/integrations-sentry-03.webp" alt="Panel de administración de Sentry" style="max-width:700px; width:100%">

Configure todos los ajustes necesarios y guarde los cambios. Después de esto, se habilitará la autorización a través de **{{projectName}}** y se desactivará el inicio de sesión mediante usuario/contraseña.
