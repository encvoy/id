---
title: "Integración de Moodle con {{projectName}} — Configuración de Inicio de Sesión Único"
description: "Aprenda a configurar el Inicio de Sesión Único en Moodle a través de {{projectName}}: configuración sencilla, protección de datos y acceso fluido para todos los empleados de la empresa."
keywords:
  - integración de Moodle con {{projectName}}
  - Moodle {{projectName}}
  - Moodle SSO
  ...
  - OAuth Moodle
  - configuración de Moodle con {{projectName}}
  - conectar Moodle a {{projectName}}
  - inicio de sesión único en moodle
...
# Cómo configurar la integración de Moodle con {{projectName}}

En esta guía, aprenderá a configurar el Inicio de Sesión Único (SSO) en **Moodle** utilizando el sistema **{{projectName}}**.
...
4. Complete los campos:  
    ...
    - **ID de cliente** — el `Client_id` de la aplicación **{{projectName}}**;
    - **Secreto del cliente** — el `Client_secret` de la aplicación **{{projectName}}**;
    - **URL base del servicio** — `<Dirección de {{projectName}}>/api/oidc`;
...
2. Si todos los datos se ingresaron correctamente, los ajustes se completarán automáticamente.  
    ...
    > Si es necesario, puede completar las URLs de los endpoints manualmente. Consulte la lista de URLs disponibles en: `https://<dirección del servicio {{projectName}}>/api/oidc/.well-known/openid-configuration`  
...
> 💡 La vinculación de un usuario de Moodle LMS con un usuario de {{projectName}} se basa en la dirección de correo electrónico. La ausencia de un correo electrónico en el perfil de {{projectName}} imposibilitará la autenticación en Moodle. Eliminar una dirección de correo previamente vinculada a Moodle en {{projectName}} y añadir una nueva dirección resultará en la creación de un nuevo usuario en Moodle.  
...
3. Asegúrese de que haya aparecido el botón **Iniciar sesión usando {{projectName}}**.
