---
title: "Inicio de sesión con OpenID Connect — Conexión en {{projectName}}"
description: "Aprenda cómo habilitar el inicio de sesión con OpenID Connect en {{projectName}}: cree un método de inicio de sesión y añádalo al widget de autorización. Conéctese en solo unos pasos."
keywords: 
  ...
  - configurar OpenID Connect en {{projectName}}
  - conectar OpenID Connect a {{projectName}}
...
# Cómo conectar el inicio de sesión con OpenID Connect en {{projectName}}
...
En esta guía, aprenderá cómo conectar la autenticación **OpenID Connect** al sistema **{{projectName}}**.
...
2. Copie los valores de los campos **Application ID/Client ID** y **Secret/Client Secret**. Los necesitará al crear la aplicación en **{{projectName}}**.
...
| **URL de redireccionamiento (Redirect URI)** (no editable) | `redirect URI` | La dirección de **{{projectName}}** a la que se redirige al usuario después de la autenticación en el servicio de terceros |
