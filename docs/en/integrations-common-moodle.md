---
title: "Moodle Integration with {{projectName}} — Setting up Single Sign-On"
description: "Learn how to set up Single Sign-On in Moodle via {{projectName}}: simple configuration, data protection, and seamless access for all company employees."
keywords:
  - Moodle integration with {{projectName}}
  - Moodle {{projectName}}
  - Moodle SSO
  ...
  - OAuth Moodle
  - configuring Moodle with {{projectName}}
  - connecting Moodle to {{projectName}}
  - single sign-on in moodle
...
# How to Configure Moodle Integration with {{projectName}}

In this guide, you will learn how to set up Single Sign-On (SSO) in **Moodle** using the **{{projectName}}** system.

> 📌 [Moodle](https://moodle.org/) is a learning management system for electronic educational courses (e-learning).

Setting up login via **{{projectName}}** consists of several key stages performed in two different systems:
...
4. Fill in the fields:  
    ...
    - **Client ID** — the `Client_id` of the **{{projectName}}** application;
    - **Client Secret** — the `Client_secret` of the **{{projectName}}** application;
    - **Service Base URL** — `<{{projectName}} Address>/api/oidc`;
...
2. If all data was entered correctly, the settings will be populated automatically.  
    ...
    > If necessary, you can fill in the endpoint URLs manually. See the list of available URLs at: `https://<{{projectName}} service address>/api/oidc/.well-known/openid-configuration`  
...
> 💡 Linking a Moodle LMS user with a {{projectName}} user is based on the email address. The absence of an email in the {{projectName}} profile will make authentication in Moodle impossible. Deleting an email address previously linked to Moodle in {{projectName}} and adding a new email address will result in the creation of a new user in Moodle.  
...
3. Ensure that the **Log in using {{projectName}}** button has appeared.
