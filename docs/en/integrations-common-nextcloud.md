---
title: "Integrating Nextcloud with {{projectName}} — Setting up Single Sign-On"
description: "Learn how to set up Single Sign-On in Nextcloud via {{projectName}}: simple configuration, data protection, and convenient access for all company employees."
keywords:
  - Nextcloud integration with {{projectName}}
  ...
  - OIDC Nextcloud
  - login to Nextcloud via {{projectName}}
  - configuring Nextcloud with {{projectName}}
  - connecting Nextcloud to {{projectName}}
...
# How to configure Nextcloud integration with {{projectName}}

In this guide, you will learn how to set up Single Sign-On (SSO) in **Nextcloud** using the **{{projectName}}** system.
...
Setting up login with **{{projectName}}** consists of two key stages performed in two different systems.
...
5. Fill in the connection parameters:
    ...
    - **Authorize url** - specify the authorization URL. For example, `https://<{{projectName}}-installation-address>/api/oidc/auth`.
    - **URL token** - specify the URL for obtaining the access token. For example, `https://<{{projectName}}-installation-address>/api/oidc/token`.
...
After completing all steps, a login button for **{{projectName}}** will be displayed in the **Nextcloud** authorization widget.
...
2. Ensure that the **Login with {{projectName}}** button has appeared.
