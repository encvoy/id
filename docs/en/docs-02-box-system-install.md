---
title: "Installing {{projectName}} via Docker — SSO Deployment"
description: "Install {{projectName}} via Docker: requirements, configuration setup, and first login. Step-by-step SSO deployment for administrators and DevOps."
keywords:
  - install {{projectName}}
...
In this guide, you will learn how to install the **{{projectName}}** SSO system via Docker on your server. We will walk through the entire process — from environment preparation to the administrator's first login.
...
Before installing the **{{projectName}}** system, ensure that your infrastructure meets the requirements.
...
For successful installation and correct operation of **{{projectName}}**, several conditions must be met:
...
**{{projectName}}** is deployed as a set of Docker containers and can be used as an enterprise OAuth 2.0 Authorization Server and OpenID Connect Provider (IdP).
...
> ⚙️ After installation, it is recommended to perform basic configuration. A detailed description of all parameters is available in the [{{projectName}} Environment Variables](./docs-03-box-system-configuration.md) section.
...
To access the **{{projectName}}** web interface, navigate to: `https://ID_HOST`.
...
2. In the mini-widget that opens, click on the service name **{{projectName}}**.
...
- [{{projectName}} System Description](./docs-01-box-about.md) — overview of **{{projectName}}** features.
- [{{projectName}} Environment Variables](./docs-03-box-system-configuration.md) — guide for preparing configuration before launch.
