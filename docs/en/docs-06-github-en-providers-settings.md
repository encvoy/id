---
title: "{{projectName}} Login Methods — Connection and Configuration"
description: "Learn how to configure login methods and the authorization widget in {{projectName}}. Quick connection of OAuth, WebAuthn, mTLS, HOTP, and TOTP providers with examples."
keywords: 
...
# How to Configure Login Methods in {{projectName}}

In this guide, you will learn how to configure login methods in **{{projectName}}**, including standard OAuth providers and enhanced authentication methods (WebAuthn, mTLS, TOTP). We will also explain how to properly set up and style the authorization widget to make the login process secure and user-friendly.
...
### Authentication Provider Types in {{projectName}}

**{{projectName}}** supports the following types of login methods:
...
Login methods can be created in different types of **{{projectName}}** dashboards:
...
For most popular services, **{{projectName}}** provides ready-made templates with settings. They simplify the connection process as they contain pre-filled parameters specific to each provider.

**The configuration process includes three steps:**

1. **Preparation:** obtain `Client ID` and `Client Secret` from the provider service.
2. **Configuration in {{projectName}}:** create a provider of the corresponding type.
...
4. Click the **Configure** button on the panel of the login method you want to edit.
    ...
    <img src="./images/settings-provider-01.webp" alt="Editing a login method in {{projectName}}" style="max-width:500px; width:100%">
...
5. Click the **Delete** button ![Delete Button](./images/button-delete.webp "Delete Button") located on the panel of the login method you want to delete.
    ...
    <img src="./images/settings-provider-02.webp" alt="Deleting a login method in {{projectName}}" style="max-width:500px; width:100%">

6. Confirm the action in the modal window.  

    <img src="./images/settings-provider-03.webp" alt="Confirming login method deletion in {{projectName}}" style="max-width:400px; width:100%">
...
1. Copy the login method settings using the **Copy** button ![Copy Button](./images/button-copy.webp "Copy Button") located on the login method panel.
    ...
    <img src="./images/settings-provider-04.webp" alt="Copying login method settings in {{projectName}}" style="max-width:500px; width:100%">  
...
The list of identifiers available for addition is formed from the login methods in the **{{projectName}}** dashboard with an active publicity setting.
...
In **{{projectName}}**, you can configure a requirement for mandatory linking of an external account identifier to the user profile. In this case, when logging into the application, a user who does not have a linked identifier will see a request to add one to their profile.
...
4. Click the **Make required** button ![Make Required Button](./images/button-required.webp "Make Required Button") on the panel of the login method you want to make mandatory.
    ...
    <img src="./images/settings-provider-05.webp" alt="Required login method in {{projectName}} user profile" style="max-width:500px; width:100%">
...
The **Login Widget** is the authorization form displayed to the user when attempting to log into an application or the **{{projectName}}** system if they are not yet authenticated.
...
<img src="./images/settings-provider-06.webp" alt="Example of {{projectName}} login widget" style="max-width:400px; width:100%">
...
5. In the second block, configure the visibility of login form elements:
    ...
    - **Hide logos of major login providers** — When enabled, hides the login method logos from the **Primary** group.
      ...
      <img src="./images/settings-provider-07.webp" alt="Configuring {{projectName}} login widget appearance" style="max-width:300px; width:100%">
...
6. In the third block, configure button design:
    ...
    - **Button font color** — Color scheme for the button text (hex code).
      ...
      <img src="./images/settings-provider-08.webp" alt="Configuring {{projectName}} widget button design" style="max-width:300px; width:100%">
...
7. If necessary, specify text:
    ...
    - **Additional field outside the widget** — Additional text that will be displayed below the widget.
      ...
      <img src="./images/settings-provider-09.webp" alt="Configuring additional text in {{projectName}} login widget" style="max-width:600px; width:100%">
...
- [Organization Management](./docs-02-box-system-install.md) — guide for working with an organization in **{{projectName}}**.
