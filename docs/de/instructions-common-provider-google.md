---
title: "Google Login — Verbindung und Konfiguration in {{projectName}}"
description: "Erfahren Sie, wie Sie den Google-Login in {{projectName}} verbinden: Erstellen Sie eine Login-Methode und fügen Sie diese zum Autorisierungs-Widget hinzu. Verbindung in nur wenigen Schritten."
keywords: 
  - Google Login 
  - Google Einrichtung in {{projectName}}
...
# So verbinden Sie den Google-Login in {{projectName}}
...
In dieser Anleitung erfahren Sie, wie Sie die Authentifizierung über ein **Google**-Konto mit dem **{{projectName}}**-System verbinden. Diese Login-Methode ermöglicht es Benutzern, sich mit ihrem **Google**-Servicekonto bei Anwendungen anzumelden.
...
Bevor Sie die Login-Methode in **{{projectName}}** konfigurieren, müssen Sie Ihre Anwendung in der **Google** Developer Console registrieren und Zugriffsschlüssel erhalten:
...
5. Erstellen Sie eine **OAuth-Client-ID**:
    ...
    - Klicken Sie auf **Erstellen**.
      ...
      > ⚠️ Nach der Erstellung erscheint ein Fenster mit den Daten: `Client-ID` und `Client-Geheimnis`. Speichern Sie diese Werte — Sie benötigen sie für die Konfiguration in **{{projectName}}**.
...
Erstellen wir nun mit den Schlüsseln von **Google** den entsprechenden Provider im **{{projectName}}**-System.
...
|---|---|---|---|
| **Name** | Der Name, der in der **{{projectName}}**-Benutzeroberfläche angezeigt wird | Text | Max. 50 Zeichen |
| **Beschreibung** | Eine kurze Beschreibung, die in der **{{projectName}}**-Benutzeroberfläche angezeigt wird | Text | Max. 255 Zeichen |
| **Logo** | Das Bild, das in der **{{projectName}}**-Benutzeroberfläche und im Login-Widget angezeigt wird | JPG, GIF, PNG oder WEBP | Max. Größe: 1 MB |
...
| **Rücksprung-URL (Redirect URI)** (nicht editierbar) | `Redirect URI` | Die **{{projectName}}**-Adresse, an die der Benutzer nach der Authentifizierung beim Drittanbieter weitergeleitet wird |
