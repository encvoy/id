---
title: "Moodle-Integration mit {{projectName}} — Einrichten von Single Sign-On"
description: "Erfahren Sie, wie Sie Single Sign-On in Moodle über {{projectName}} einrichten: einfache Konfiguration, Datenschutz und nahtloser Zugriff für alle Unternehmensmitarbeiter."
keywords:
  - Moodle-Integration mit {{projectName}}
  - Moodle {{projectName}}
  - Moodle SSO
  ...
  - OAuth Moodle
  - Moodle mit {{projectName}} konfigurieren
  - Moodle mit {{projectName}} verbinden
  - Single Sign-On in Moodle
...
# So konfigurieren Sie die Moodle-Integration mit {{projectName}}

In dieser Anleitung erfahren Sie, wie Sie Single Sign-On (SSO) in **Moodle** mithilfe des **{{projectName}}**-Systems einrichten.

> 📌 [Moodle](https://moodle.org/) ist ein Lernmanagementsystem für elektronische Bildungskurse (E-Learning).

Die Einrichtung des Logins über **{{projectName}}** besteht aus mehreren Schlüsselphasen, die in zwei verschiedenen Systemen durchgeführt werden:
...
4. Füllen Sie die Felder aus:  
    ...
    - **Client-ID** — die `Client_id` der **{{projectName}}**-Anwendung;
    - **Client-Geheimnis** — das `Client_secret` der **{{projectName}}**-Anwendung;
    - **Service-Basis-URL** — `<{{projectName}}-Adresse>/api/oidc`;
...
2. Wenn alle Daten korrekt eingegeben wurden, werden die Einstellungen automatisch ausgefüllt.  
    ...
    > Falls erforderlich, können Sie die Endpunkt-URLs manuell ausfüllen. Die Liste der verfügbaren URLs finden Sie unter: `https://<{{projectName}}-Service-Adresse>/api/oidc/.well-known/openid-configuration`  
...
> 💡 Die Verknüpfung eines Moodle-LMS-Benutzers mit einem {{projectName}}-Benutzer basiert auf der E-Mail-Adresse. Das Fehlen einer E-Mail im {{projectName}}-Profil macht die Authentifizierung in Moodle unmöglich. Das Löschen einer zuvor mit Moodle verknüpften E-Mail-Adresse in {{projectName}} und das Hinzufügen einer neuen E-Mail-Adresse führt zur Erstellung eines neuen Benutzers in Moodle.  
...
3. Stellen Sie sicher, dass die Schaltfläche **Anmelden mit {{projectName}}** erschienen ist.
