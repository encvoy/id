---
title: "Inscription dans {{projectName}} — connexion et récupération de mot de passe"
description: "Découvrez comment vous inscrire dans {{projectName}}, créer un compte et récupérer votre mot de passe. Suivez le guide étape par étape et commencez à utiliser le service !"
keywords:
  - inscription dans {{projectName}}
  - s'inscrire dans {{projectName}}
  - création de compte {{projectName}}
  - connexion à {{projectName}}
  - se connecter à {{projectName}}
  - récupération de mot de passe {{projectName}}
  - réinitialiser le mot de passe {{projectName}}
  - connexion OAuth à {{projectName}}
  - connexion sociale {{projectName}}
  - authentification {{projectName}}
  - autorisation {{projectName}}
...
# Inscription, Connexion et Récupération de mot de passe dans {{projectName}}

Dans ce guide, vous apprendrez comment créer un compte dans **{{projectName}}**, vous connecter à l'aide d'un nom d'utilisateur et d'un mot de passe ou via des services externes, et récupérer l'accès à votre profil en toute sécurité si nécessaire.
...
<img src="./images/user-auth-01.webp" alt="Bouton de création de compte dans le widget de connexion {{projectName}}" style="max-width:400px; width:100%">

2. **Sur le formulaire de sélection d'action lors de la connexion via une méthode d'authentification**, si l'identifiant spécifié n'est lié à aucun profil dans **{{projectName}}**

<img src="./images/user-auth-02.webp" alt="Bouton de création de compte dans le widget de sélection d'action {{projectName}}" style="max-width:400px; width:100%">

> 💡 **Le design du formulaire** peut varier en fonction des paramètres de l'application spécifique.

### Comment créer un compte dans {{projectName}}
...
2. Saisissez les données requises dans le formulaire d'inscription.
   ...
   <img src="./images/user-auth-03.webp" alt="Formulaire de saisie du nom d'utilisateur lors de l'inscription dans le widget {{projectName}}" style="max-width:400px; width:100%">
...
4. Saisissez le code ou suivez le lien figurant dans l'e-mail envoyé à l'adresse spécifiée.
   ...
   <img src="./images/user-auth-04.webp" alt="Ajout d'une adresse e-mail dans le widget {{projectName}}" style="max-width:400px; width:100%">
...
Vous pouvez vous autoriser dans l'application en utilisant votre compte **{{projectName}}**.
...
<img src="./images/user-auth-05.webp" alt="Widget de connexion {{projectName}}" style="max-width:400px; width:100%">
...
2. Saisissez votre mot de passe à la deuxième étape et cliquez sur **Se connecter**.
   ...
   <img src="./images/user-auth-06.webp" alt="Saisie du mot de passe dans le widget de connexion {{projectName}}" style="max-width:400px; width:100%">
...
- lors de la première connexion, un formulaire de confirmation d'accès aux données s'ouvrira ;
  ...
  Exemple d'un formulaire demandant l'accès aux données du profil :
    ...
    <img src="./images/user-auth-07.webp" alt="Demande d'accès aux données du profil dans le widget de connexion {{projectName}}" style="max-width:400px; width:100%">
...
- si l'application requiert des champs de profil obligatoires, le système demandera de les remplir ;
  ...
  Exemple de demande de numéro de téléphone :
    ...
    <img src="./images/user-auth-08.webp" alt="Demande de complétion de champ dans le widget de connexion {{projectName}}" style="max-width:400px; width:100%">
...
- si les données sont masquées par les [paramètres de confidentialité](./docs-12-common-personal-profile.md#privacy-levels), vous serez invité à modifier le niveau d'accès.
  ...
  Exemple de modification de la confidentialité de la date de naissance :
    ...
    <img src="./images/user-auth-09.webp" alt="Demande de modification de confidentialité de la date de naissance dans le widget de connexion {{projectName}}" style="max-width:400px; width:100%">
...
<img src="./images/user-auth-10.webp" alt="Méthodes de connexion dans le widget de connexion {{projectName}}" style="max-width:400px; width:100%">
...
Si vous vous êtes déjà connecté à **{{projectName}}** dans votre navigateur, une nouvelle autorisation ne sera pas nécessaire.

1. Lors de l'accès, une fenêtre de sélection d'un compte enregistré s'ouvrira.
2. Cliquez sur le nom d'utilisateur.

<img src="./images/user-auth-11.webp" alt="Widget de sélection de compte {{projectName}}" style="max-width:400px; width:100%">
...
Si vous avez oublié le mot de passe de votre compte **{{projectName}}**, vous pouvez facilement le récupérer.

### Comment récupérer un mot de passe dans {{projectName}}

1. À la première étape du widget de connexion, saisissez vos données d'identification (ex: nom d'utilisateur, e-mail ou numéro de téléphone) et cliquez sur **Se connecter**.
2. À l'étape suivante, sélectionnez **Récupérer le mot de passe**.

<img src="./images/user-auth-12.webp" alt="Bouton de récupération de mot de passe de compte dans le widget de connexion {{projectName}}" style="max-width:400px; width:100%">
...
4. Saisissez le code reçu par e-mail.
   ...
   <img src="./images/user-auth-13.webp" alt="Saisie du code lors de la récupération de mot de passe dans le widget de connexion {{projectName}}" style="max-width:400px; width:100%">
