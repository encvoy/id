---
title: "Login Google — Connessione e Configurazione in {{projectName}}"
description: "Scopri come connettere il login Google in {{projectName}}: crea un metodo di accesso e aggiungilo al widget di autorizzazione. Connettiti in pochi passaggi."
keywords: 
  - Google login 
  - configurazione Google in {{projectName}}
  - autenticazione Google 
  - connettere Google
  - Google login {{projectName}}
  - Google OAuth {{projectName}}
  - Google sign-in
  - autorizzazione Google
  - Google {{projectName}}
  - login tramite Google {{projectName}}
...
# Come Connettere il Login Google in {{projectName}}
...
In questa guida imparerai come connettere l'autenticazione tramite un account **Google** al sistema **{{projectName}}**. Questo metodo di accesso consente agli utenti di accedere alle applicazioni utilizzando il proprio account dei servizi **Google**.
...
Prima di configurare il metodo di accesso in **{{projectName}}**, devi registrare la tua applicazione nella console per sviluppatori di **Google** e ottenere le chiavi di accesso:
...
5. Crea un **OAuth Client ID**:
    ...
    - Clicca su **Create**.
      ...
      > ⚠️ Dopo la creazione, apparirà una finestra con i dati: `Client ID` e `Client Secret`. Salva questi valori: ti serviranno durante la configurazione in **{{projectName}}**.
...
Ora, con le chiavi ottenute da **Google**, creiamo il provider corrispondente nel sistema **{{projectName}}**.
...
|---|---|---|---|
| **Nome** | Il nome che verrà visualizzato nell'interfaccia del servizio **{{projectName}}** | Testo | Max 50 caratteri |
| **Descrizione** | Una breve descrizione che verrà visualizzata nell'interfaccia del servizio **{{projectName}}** | Testo | Max 255 caratteri |
| **Logo** | L'immagine che verrà visualizzata nell'interfaccia del servizio **{{projectName}}** e nel widget di login | JPG, GIF, PNG o WEBP | Dimensione max: 1 MB |
...
| **URL di reindirizzamento (Redirect URI)** (non modificabile) | `Redirect URI` | L'indirizzo **{{projectName}}** al quale l'utente viene reindirizzato dopo l'autenticazione nel servizio di terze parti |
